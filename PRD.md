...
5. **AI strategy generation** — LLM call grounded in business/product/objective, stored strategy record *(done)*
   ...
   **TEST_PLAN is a structured advertising experiment, not just two recommended audiences (confirmed 2026-09-01, "Phase A" of a larger redesign — Phase B, an Optimizer that consumes this plan against real results, and Phase C, real two-adset Meta publishing, are follow-ups, not yet built):**
   ...
   - **Budget/duration are a backend business rule, not an LLM recommendation or an absolute advertising truth** (confirmed 2026-09-01): `_compute_test_daily_budget` (`app/services/strategist.py`) uses `DEFAULT_TEST_BUDGET` ($50/day) and `DEFAULT_TEST_DURATION` (10 days) when the product has no computable unit economics; otherwise the daily budget is anchored to the product's own `target_cac` (a productive test should buy roughly one conversion a day at that rate), clamped to `[MIN_TEST_BUDGET, MAX_STARTING_BUDGET]` = `[$20, $75]`/day (constants in `app/schemas/strategy.py`). The LLM is told the resulting numbers as context, never asked to set them. `totalBudget` is backend-computed (`dailyBudget × durationDays`).
